...
function requireFlag(name: string, expected: boolean) {
  ...
  if (actual !== expected) {
    throw new Error(`${name} must be ${String(expected)} for mainnet factory deploy`);
  }
}
...
if ((process.env.OG_NETWORK ?? "").toLowerCase() !== "mainnet") {
  throw new Error("Mainnet factory deploy requires OG_NETWORK=mainnet");
}
if (Number(requireEnv("OG_CHAIN_ID")) !== MAINNET_CHAIN_ID) {
  throw new Error(`Mainnet factory deploy requires OG_CHAIN_ID=${MAINNET_CHAIN_ID}`);
}
...
if (deployerBalance < parseEther("0.005")) {
  throw new Error("Deployer needs at least 0.005 0G for mainnet factory deploy gas");
}
...
const outputPath = join(".data", "deployments", "mainnet-policy-vault-factory.json");
await writeDeployment(outputPath, output);

console.log("0G mainnet PolicyVaultFactory deployed. Redacted deployment artifact:", outputPath);
console.log({
  chainId,
  factory: factory.address,
  fromBlock: fromBlock.toString(),
