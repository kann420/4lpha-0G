              ...
              <section className="rounded-[24px] border border-white/8 bg-[linear-gradient(135deg,rgba(11,16,23,0.96),rgba(6,10,15,0.84))] px-4 py-5 shadow-[0_28px_100px_rgba(0,0,0,0.24)] lg:rounded-[30px] lg:px-8 lg:py-6">
                <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                  <div className="max-w-[20rem] space-y-2.5">
                    <p className="text-[11px] uppercase tracking-[0.32em] text-cyan-200/60">
                      Agent workspace
                    </p>
                    <h1 className="text-2xl font-semibold tracking-tight text-white sm:whitespace-nowrap sm:text-3xl lg:text-4xl">
                      Create your Agent
                    </h1>
                  ...
                  <div className="flex flex-col gap-3 sm:items-start lg:min-w-[24.25rem] lg:items-end">
                    <div className="flex flex-nowrap items-center gap-2.5 lg:gap-3">
                      ...
                      <button
                        type="button"
                        className="inline-flex h-12 items-center rounded-full border border-white/10 bg-white/[0.04] px-4 text-sm font-medium text-slate-200 transition-colors hover:bg-white/[0.06]"
                      >
...
function HealthStrip({
  ...
  return (
    <section className="grid gap-3 rounded-[24px] border border-white/8 bg-[linear-gradient(180deg,rgba(12,17,24,0.94),rgba(8,12,18,0.9))] p-3 shadow-[0_20px_60px_rgba(0,0,0,0.22)] sm:grid-cols-2 lg:rounded-[28px] lg:p-4 xl:grid-cols-5">
      {items.map((item) => {
        const toneClass =
          item.tone === "positive" ? "text-emerald-300" : item.tone === "warning" ? "text-amber-200" : "text-slate-400";
        return (
          <article
            key={item.label}
            className="rounded-[18px] border border-white/8 bg-white/[0.03] px-3 py-3 lg:rounded-[22px] lg:px-4 lg:py-3.5"
          >
...
function AgentCard({
  ...
  return (
    <article
      className="animate-feed-reveal rounded-[24px] border border-white/8 bg-[linear-gradient(180deg,rgba(12,17,24,0.95),rgba(7,10,15,0.86))] p-4 shadow-[0_20px_60px_rgba(0,0,0,0.2)]"
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm text-slate-400">{agent.strategy}</p>
          </div>
        </div>
      ...
      <div className="mt-4 rounded-[18px] border border-white/8 bg-white/[0.03] px-3 py-2.5">
        <div className="flex items-center justify-between gap-3">
          <span className={`rounded-full border px-2 py-0.5 text-[11px] font-medium capitalize ${riskClass}`}>
            {agent.risk} risk
          </span>
          <span className="font-mono text-[11px] text-slate-500">proof-bound</span>
        </div>
        <p className="mt-2 text-sm leading-6 text-slate-400">{agent.note}</p>
      </div>
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-white/8 pt-4">
        <button
          ...
          className="inline-flex h-9 items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-3 text-sm text-slate-200 transition-colors hover:bg-white/[0.07]"
        >
          {busy ? <Sparkles className="h-4 w-4 text-cyan-200" /> : armed ? <Pause className="h-4 w-4" /> : <Play className="h-4 w-4" />}
          {armed ? "Pause" : "Arm"}
        </button>
        <button
          type="button"
...
function AgentMetric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-[16px] border border-white/8 bg-black/20 px-3 py-2">
      ...
      <p className="mt-1 truncate font-mono text-xs font-semibold text-slate-100">{value}</p>
    </div>
  );
}
